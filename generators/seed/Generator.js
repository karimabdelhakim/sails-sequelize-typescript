...
module.exports = {
  ...
  before: function (scope, cb) {
    ...
    //
    // e.g. if someone runs:
    // $ sails generate seed user find create update
    // then `scope.args` would be `['user', 'find', 'create', 'update']`
    if (!scope.args[0]) {
      return cb(new Error('Please provide a name for this seeder.'));
    }
    ...
    myApp.load({ ssl: {}, port: 1337, models: { migrate: 'safe' } }, (err) => {
      if (err) throw err;
...
function INVALID_SCOPE_VARIABLE(varname, details, message) {
  var DEFAULT_MESSAGE =
    'Issue encountered in generator "seed":\n' +
    'Missing required scope variable: `%s`"\n' +
    'If you are the author of `sails-generate-seed`, please resolve this ' +
