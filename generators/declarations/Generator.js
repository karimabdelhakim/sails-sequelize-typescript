  ...
  before: function (scope, cb) {
    ...
    myApp.load({}, (err) => {
      ...
      myApp.lower((err) => {
        ...
        console.log('Generated Declaration files Successfully');
      });
  ...
  glob('api/models/**/*.ts', (err, matches) => {
    ...
    for (path of matches) {
      ...
      for (attribute in attributes) {
        attrib_type += `${attribute} : ${attributes[attribute].type}; `;
      }

      let model_name_array = path.split('/');
      let model_name = model_name_array[model_name_array.length - 1];
      if(model_name==='index')
        continue;
    ...
    //<models here>
      ...
      /*let model_interface = `var ${model_name}: Model
      export interface ${model_name} extends dynamicObjects {
        ${attrib_type}
      }
      //<models here>`;*/
      addToModelsService(model_def, '//<models here>', `get ${model_name}()`);
      //addToGlobalDeclarationFile(model_interface, '//<models here>', `var ${model_name}: Model`);
    }
